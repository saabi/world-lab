...
- **group function round-trip (`functionToGroup`)** — inverse of `groupToFunction` not built: decompose a group WGSL `fn` (restricted `let` + primitive calls + frontmatter) back into a `GroupDefinition` subgraph for CodeView save and TouchDesigner-style drill-in. Use.GPU linker (`@use-gpu/shader`) is proposed as parse/symbol/dep validation only; node separation belongs in a dedicated compiler pass on the **group emission grammar**, not in the linker itself. Profile A (no dynamic `for` loops) first; built-in groups should prefer stored subgraph JSON over inversion. **Proposal for review:** `architecture/procedural-graph/group-function-round-trip-proposal.md`.
...
- Functions representing group nodes must be decomposable into their components and editable on request. Built-in group functions such as `math.remap` must be inspectable as graphs (TouchDesigner-style zoom-in) and automatically cloned and replaced if modified. Analysis and phased plan: `architecture/procedural-graph/group-function-round-trip-proposal.md` (depends on node groups UX above).
...
- **stream graphs (CPU-first)**: typed `stream<T>`, `future<T>`, and `signal<T>` ports; multi-emitter pipeline primitives; mux/demux/filter/window; promise nodes; `sink.host` graph→host egress (context-specific signal drain). Spec: `architecture/procedural-graph/stream-graphs.md` (phased A→D). **Blocked on E1** (`cpu-handle`, stream/future/signal TypeRefs).
- **preview monitors (TouchDesigner-style probes)**: temporary preview taps on arbitrary output ports via port context menu — no `target.display` / `preview.fieldSink` ghost nodes; `PreviewProbe` in editor chrome unioned into `enumeratePreviewBuffers`. Spec: `architecture/procedural-graph/preview-monitors.md` (phased A→C). Composes with stream `sink.host` for incremental pane updates.
- **picking and collision (graph consumers)**: Mode A `spatial.rayField`; Mode B `spatial.ray`; `signal<SpatialHit>`; walk via `spatial.buildHeightfield` + `spatial.sampleHeight`. Spec: `architecture/procedural-graph/picking-and-collision.md` (phased A→D). **Blocked on E1/E5**.
- **mesh geometry, physics, and navigation**: partition kinds (solid/walkable/freeSpace), freeSpace input + LOS contract, `spatial.pathPortals` → `SpatialPath`. Spec: `architecture/procedural-graph/mesh-geometry-and-navigation.md` (phased A→E). **Blocked on E1/E4/E5**.
- **CPU elemental model (spec consolidation)**: ADR **accepted** — rev. 3 (2026-07-04): F2-aligned `cpu-handle`, `ResourceShape` CPU variant, revision on `ResourceBinding`, `HostBinding` + `HostEgressBinding`. ADR: `architecture/procedural-graph/cpu-elemental-model.md`; memo rev. 2–3: `architecture/procedural-graph/spec-consolidation-2026-07.md`. **E1 may route** after rev. 3 doc merge; no `packages/graph` code until explicitly routed.
