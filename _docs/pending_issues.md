...
- **stream graphs (CPU-first)**: typed `stream<T>`, `future<T>`, and `signal<T>` ports; multi-emitter pipeline primitives; mux/demux/filter/window; promise nodes (`async.spawn`, `async.await`, `async.awaitAll`); `sink.host` graph→host egress for UI/scene (subscriptions, rAF batching) — distinct from render [pipeline-as-graph.md](architecture/procedural-graph/pipeline-as-graph.md). Spec: `architecture/procedural-graph/stream-graphs.md` (phased A→D). See `elemental-webgpu-architecture-review.md` "Roadmap realignment §Stream graphs".
...
- **picking and collision (graph consumers)**: procedural mesh/field ray pick and heightfield collision using the **same** `position`/`normal` subgraph as mesh preview/render; `pick.mesh` consumer, `signal<PickResult>` on click/hover; continuous walk via heightfield **block** buffer + `collision.sample` (not per-frame streams). Planet `renderPickingPass` still stub. Spec: `architecture/procedural-graph/picking-and-collision.md` (phased A→D; Phase A WebGPUToy mesh-gen CPU pick is Foundation-independent). See `elemental-webgpu-architecture-review.md` "Roadmap realignment §Picking and collision".
