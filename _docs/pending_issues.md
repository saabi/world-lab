...
- **stream graphs (CPU-first)**: typed `stream<T>` and `future<T>` ports, multi-emitter pipeline primitives (e.g. NER → companies/people/addresses), mux/demux/filter/window operators, and promise-shaped command nodes (`async.spawn`, `async.await`, `async.awaitAll`, `tryAwait`) over a worker pool — distinct from render [pipeline-as-graph.md](architecture/procedural-graph/pipeline-as-graph.md) and complementary to audio block scheduling. IR has multi-output ports and `command` primitive kind but no `stream`/`future` `TypeRef`, stream consumer, or operator catalog. Spec: `architecture/procedural-graph/stream-graphs.md` (phased A→D; Phase A is Foundation-independent like audio Phase A; Phase D optional GPU viz gated on resource GPU binds). See `elemental-webgpu-architecture-review.md` "Roadmap realignment §Stream graphs".
- **preview monitors (TouchDesigner-style probes)**: temporary preview taps on arbitrary output ports via port context menu — no `target.display` / `preview.fieldSink` ghost nodes; `PreviewProbe` in editor chrome (`GraphArtifact` / session layout) unioned into `enumeratePreviewBuffers`, reusing existing CPU/GPU/mesh/audio panels. Buffer-list preview landed (`M-preview-buffer-list.md`); probes are the deferred node-probe follow-on. Spec: `architecture/procedural-graph/preview-monitors.md` (phased A→C; Foundation-independent editor work). See `elemental-webgpu-architecture-review.md` "Roadmap realignment §Preview monitors".
