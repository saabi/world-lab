...
| [f3.6-pipeline-kernels-design.md](./f3.6-pipeline-kernels-design.md) | **Design discussion (2026-07-08), not a contract:** dynamic per-document binding sets, real varying dataflow, presentation-target compatibility, and what (if anything) replaces `stage.vertex`/`stage.fragment` — four sub-problems investigated against the current codebase, options compared, tentative F3.6.1–4 sequencing sketched | `graph`, `compiler`, `runtime-webgpu` |
