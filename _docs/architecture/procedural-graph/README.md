...
| [foundation-3-generic-kernels-plan.md](./foundation-3-generic-kernels-plan.md) | **Sequencing plan (2026-07-05), draft:** typed vertex/fragment/compute kernels, arbitrary resource bindings (incl. the missing storage-read-write binding kind), typed varyings closing the Mode A vertex-displacement gap, and real compute dispatch — the already-reserved `{kind:'kernel'}` placeholder gets its first real content | `graph`, `compiler`, `runtime-webgpu` |
