...
| [lights-and-materials-design.md](./lights-and-materials-design.md) | **Design discussion, not a contract:** lights as typed source nodes feeding a `list<Light>` port by multi-fan-in (the landed tuple mechanism — no mandatory collect node), lighting as library not engine (`material.pbr` is an ordinary group; unlit shaders unaffected), the camera-parallel duality (graph-authored → static unroll now; `host.sceneLights` from `collectSceneLights` → storage-buffer loop in the scene era). The one real engine gap: struct-valued list ports (L1, not light-specific — F4.4 draw lists and F4.5 particles need it too). Tentative L1-L3 phasing; L2 gated on F4.2 | `graph`, `compiler`, `runtime-webgpu`, `graph-editor` |
