...
| [F2.1-resource-type-algebra.md](./F2.1-resource-type-algebra.md) | Foundation 2, milestone 1: real `ResourceDescriptor` shape (lifetime-qualified wrapper over F1.5's existing `TypeRef` buffer/texture/sampler variants), real `BufferUsage` union replacing the placeholder string, minimal usage-inference from graph wiring — pure types only, no GPU allocation | 📌 ready to route | Cursor or Codex |
