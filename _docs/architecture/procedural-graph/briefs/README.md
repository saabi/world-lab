...
| [F2.5-foundation-2-proof.md](./F2.5-foundation-2-proof.md) | Foundation 2, final milestone: two bundled, pickable samples proving the resource-history model — two-pass texture feedback (new `input.channel` host-input primitive + WGSL `textureSample` emission, using the already-reserved `HostBinding{context:'read-resource'}` and already-existing `BindingDecl` texture/sampler kinds; `layout:'auto'` needs no bind-group-layout code) and buffer feedback (new `BindingDecl.kind:'storage-read-write'`, hand-written fragment entry via the existing `entryOverride` escape hatch — not compute dispatch). A small additive `resolveChannelDependencies`/`buildPassGraphWithChannelReads` pair augments the plan only when channel wiring is detected; `buildIndependentPassGraph` stays untouched for the zero-read case | 📌 ready to route | Cursor or Codex |
