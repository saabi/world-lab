...
| [F3.1-kernel-binding-type-algebra.md](./F3.1-kernel-binding-type-algebra.md) | Foundation 3, milestone 1: gives `{kind:'kernel', stage}` its first real shape via a graph-level `KernelBindingTemplate` (shader name, binding index, resource type, access, stage visibility) kept independent of the compiler's `BindingDecl`, joined by a pure `bindingDeclKindForTemplate` translation; `resolveKernelBindings` specifies exactly how static, id-less binding templates map to per-pass `ResourceBinding.resourceId` (`Pass.bindings`'s first consumer beyond usage inference); `BindingDecl.kind` gains the missing `storage-read-write` variant F2.5's buffer-feedback sample had to route around; validation enforced at both registration time and resolution time via one shared function. Pure types + pure resolution functions only — no GPU allocation, no dispatch | 📌 ready to route | Cursor or Codex |
