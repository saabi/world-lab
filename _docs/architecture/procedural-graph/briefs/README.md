...
| [F2.1-resource-type-algebra.md](./F2.1-resource-type-algebra.md) | Foundation 2, milestone 1 (rev. 2): id-less `ResourceTemplate`/instance-level `ResourceInstance` split (fixes the same static-vs-per-node mistake F1.3 caught for `SinkDefinition`), explicit `ResourceAccess`/`ResourceBinding` so `inferBufferUsage` aggregates real per-binding access instead of inventing it from topology, real `BufferUsage` union — pure types only, no GPU allocation | 📌 ready to route | Cursor or Codex |
