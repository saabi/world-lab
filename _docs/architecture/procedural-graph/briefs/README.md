...
| [F3.2-typed-varyings.md](./F3.2-typed-varyings.md) | Foundation 3, milestone 2: extends `assembleStageEntry`'s vertex/fragment templates with a shared, typed varyings struct beyond `position` (`VaryingDecl`, `varyingsStructWgsl`, `varyingsStructName`) — the direct fix for the Mode A vertex-displacement gap's missing data-flow piece — plus `assertVaryingsMatch`, a single clear rejection point for a vertex/fragment pair that disagrees on what's shared (F2.5's strict-equality "reject the mismatch clearly" precedent). Compiler-internals only: no dependency on F3.1's landed types, no `packages/graph` changes, no wiring into the real render pipeline (the fixed plane/grid path stays untouched — that's F3.4's job), no bundled sample (nothing rendered changes). Proven to compile on a real device, not just string-matched | 📌 ready to route | Cursor or Codex |
