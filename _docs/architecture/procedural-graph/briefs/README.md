...
| [F2.2-resource-dependency-planner.md](./F2.2-resource-dependency-planner.md) | Foundation 2, milestone 2: generalizes `frameGraph/types.ts` to F2.1's resource union (`RenderTarget`→`ResourceTarget`, samplers excluded from targets at the type level; `previousFrame`→`version`, newly validated against `history` lifetime), parallel `resolveBufferSizes` alongside `resolveTargetSizes`, `collectResourceInstances` materializes `ResourceInstance`s from a real `GraphDocument`, `Pass.bindings` reserved (unconsumed) — no algorithmic change to the ordering core | 📌 ready to route | Cursor or Codex |
